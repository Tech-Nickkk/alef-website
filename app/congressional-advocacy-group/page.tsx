...
const MemberCard = ({ member }: { member: any }) => (
    <div className="group relative bg-white/5 border border-black/5 rounded-xl overflow-hidden hover:shadow-2xl transition-all duration-500">
        <div className="aspect-[4/5] relative overflow-hidden bg-gray-100">
            <Image
                src={`/congressionaladvocasygroup/${member.img}`}
                alt={member.name}
                fill
            ...
            <div className="absolute inset-x-0 bottom-0 h-4/5 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-90 transition-opacity duration-500" />
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 text-white z-10">
                ...
                <div className="transform transition-transform duration-500 lg:group-hover:-translate-y-1">
                    <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-bebas text-3xl tracking-wide drop-shadow-md">{member.name}</h3>
...
export default function CongressionalAdvocacyGroupPage() {
    return (
        <main className="bg-background min-h-screen pb-24">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 mb-24">
                <div className="flex items-center gap-4 mb-12">
                    ...
                    <h2 className="font-bebas text-4xl md:text-5xl text-foreground/90">United States Senate</h2>
                    <span className="h-px flex-1 bg-black/10"></span>
                </div>

                {/* Republicans */}
                <div className="mb-16">
                    <h3 className="font-oswald text-2xl text-red mb-8 uppercase tracking-widest pl-4 border-l-4 border-red">Republicans</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {SENATE_REPUBLICANS.map((member, i) => (
                ...
                {/* Democrats */}
                <div>
                    <h3 className="font-oswald text-2xl text-red mb-8 uppercase tracking-widest pl-4 border-l-4 border-red">Democrats</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {SENATE_DEMOCRATS.map((member, i) => (
                            <MemberCard key={i} member={member} />
                        ))}
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12 mb-24">
                <div className="flex items-center gap-4 mb-12">
                    <span className="h-px flex-1 bg-black/10"></span>
                    <h2 className="font-bebas text-4xl md:text-5xl text-blue">House of Representatives</h2>
                    <span className="h-px flex-1 bg-black/10"></span>
                </div>
                ...
                <div className="mb-16">
                    <h3 className="font-oswald text-2xl text-red mb-8 uppercase tracking-widest pl-4 border-l-4 border-red">Republicans</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {HOUSE_REPUBLICANS.map((member, i) => (
                            <MemberCard key={i} member={member} />
                        ))}
                    </div>
                </div>

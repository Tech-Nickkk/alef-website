...
export default async function BlogsAndArticles() {
    ...
    // Fallback if no blog is found (optional, but good for safety)
    if (!blog) return null;
                    ...
                    <div className="bg-blue border border-white/10 rounded-2xl p-8 md:p-12 flex flex-col justify-between min-h-[500px] h-full relative group hover:border-white/30 transition-colors duration-500">
                        ...
                        <div className="flex flex-col flex-1">
                            ...
                            <div className="flex justify-between items-center mb-8 w-full">
                                <div className="bg-red text-white px-4 py-1.5 rounded-full flex items-center gap-2 shadow-[0_0_15px_rgba(220,38,38,0.5)]">
                                    <Siren className="w-3 h-3 md:w-4 md:h-4" />
                                    <span className="font-oswald text-xs md:text-xs tracking-widest uppercase font-bold">LATEST ANALYSES</span>
                                </div>
                                <span className="font-oswald text-xs tracking-widest text-white/60">OPINION // {blog.author?.name?.toUpperCase() || 'ALEF TEAM'}</span>
                            </div>

...
import { MediaSubmissionTemplate } from '@/app/components/MediaSubmissionTemplate';
...
export async function POST(req: Request) {
    try {
        ...
        const emailHtml = await render(
            React.createElement(MediaSubmissionTemplate, {
                name,
                email,
                title,
                url,
        ...
        const data = await resend.emails.send({
            from: fromEmail,
            to: recipients,
            subject: `New Media Submission: ${title}`,
            replyTo: email,
            html: emailHtml,

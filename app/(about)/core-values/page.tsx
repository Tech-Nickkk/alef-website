            ...
            <main className="grow pt-32 px-4 md:px-8 lg:px-12 max-w-[1400px] mx-auto w-full z-10 relative">
                ...
                <div className="mb-20 text-center max-w-4xl mx-auto">
                    <AnimatedTitle
                        ...
                        className="text-6xl md:text-8xl font-bebas text-theme-white mb-8 justify-center flex"
                    />
                    <div className="h-1 w-24 bg-theme-accent mx-auto mb-8"></div>
                    <p className="font-oswald text-xl md:text-2xl text-theme-white/70 leading-relaxed">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
                    {values.map((val, index) => (
                        <div key={index} className="bg-card-bg border border-theme-white/10 hover:border-theme-white/30 rounded-2xl p-6 flex flex-col items-center text-center transition-all duration-300 group">
                            <div className="text-white/50 group-hover:text-theme-accent transition-colors duration-300 mb-6">
                                {val.icon}
                            </div>
                            <h3 className="text-2xl font-bebas text-white mb-4 group-hover:text-theme-accent transition-colors">{val.title}</h3>
                            <p className="font-oswald text-white/70 leading-relaxed">
                                {val.desc}
                            </p>
                            <span className="mt-8 text-5xl font-bebas text-theme-white/5 group-hover:text-theme-white/10 transition-colors">
                                0{index + 1}
                            </span>
                        </div>
                    ))}
                ...
                <section className="mb-20 pt-10 border-t border-theme-white/10">
                    <div className="flex items-end justify-between mb-10">
                        <div>
                            <h2 className="text-4xl font-bebas text-theme-white mb-2">Strategic Pillars</h2>
                            <p className="font-oswald text-theme-white/50 text-lg">The foundation of our movement</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {pillars.map((pillar, idx) => (
                            <div key={idx} className="flex items-center p-6 bg-card-bg border border-theme-white/10 rounded-xl hover:border-theme-accent/50 transition-all duration-300 gap-8">
